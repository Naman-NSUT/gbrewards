...
/**
 * The same posters the worker app shows, published once from the same back
 * office. The list needs a dealer token; the image bytes behind `image_url` are
 * public so <Image> can load them without one.
 */
export async function listBanners(): Promise<Banner[]> {
  const resp = await api.get<Banner[]>('/dealer/banners');
  return resp.data;
}
