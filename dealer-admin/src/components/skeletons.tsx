...
function Bar({ w = '100%', h = 14, r = 6 }: { w?: number | string; h?: number; r?: number }) {
  return <div className="dr-skeleton" style={{ width: w, height: h, borderRadius: r }} />;
}

export function StatSkeleton() {
  return (
    <div
      style={{ border: `1px solid ${brand.border}`, borderRadius: 12, padding: 20, minHeight: 132 }}
    >
      <Bar w={90} h={11} />
      <div style={{ height: 16 }} />
      <Bar w={70} h={28} />
...
/** Content-shaped, not a spinner: the table's own geometry appears first so the
